...
| BC content paths | `engine/paths.py`, `docs/superpowers/specs/2026-09-05-bc-path-resolution-design.md`, `docs/superpowers/specs/2026-09-06-first-run-folder-picker-design.md` | Where `game/` and `sdk/` live. Five sources, **in precedence order**: a root picked in the first-run screen, `--game-dir`/`--sdk-dir`, `DAUNTLESS_GAME_DIR`/`DAUNTLESS_SDK_DIR`, `settings.json`, legacy in-project. The highest **set** one wins even when invalid. A pick outranks the CLI flag on purpose — it is the most recent explicit human act, and an invalid flag stays "set", so anything lower would lose to the very flag the screen exists to correct. Resolved at USE, never at import — a module-level constant is stale the moment the first-run picker changes a root. When no source resolves, boot hands off to the CEF-rendered "Select Bridge Commander Install" screen (`engine/ui/first_run_panel.py`, driven from `engine/host_loop.py`'s own pump loop before the game loop exists) rather than a bare dialog. Guarded by `tests/unit/test_path_indirection.py`. |
...
| Collision scuff decals | `engine/appc/collisions.py:scuff_radius_gu`, `native/src/renderer/shaders/opaque.frag:apply_scuffs`, `native/src/renderer/scuff_texture.cc`, `docs/superpowers/specs/2026-09-20-collision-scuff-normal-decals-design.md` | Collisions route `weapon_type="collision"` → `WeaponClass::Scuff` (2): a **splatted normal map** (`native/assets/textures/scuff_normal.tga`, a PROJECT asset resolved via `set_project_asset_root`, never `game/`) sampled per scuff in its slip frame at a hashed offset, plus bare metal on the creases and a grime fill, in a **pre-lighting** pass on `n_shade`/`base`; `textureGrad` with gradients taken **outside** the loop. ⚠️ Procedural relief was tried eight ways live (scratches, buckle, dished facets, mesh-oriented panels) and always read as a stencil or a grid — don't restart it. ⚠️ The scuff size travels as `decal_radius` — never `splash_radius`, which sets the subsystem-damage catchment. ⚠️ The post-lighting scorch loop must `continue` on class 2 or the ember comes back. `kScuff*` consts: rebuild to tune. Live tuning: `--developer` → Damage Preview / Collision Sim. |
