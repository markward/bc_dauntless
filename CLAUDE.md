...
| **Stub heatmap — CHECK BEFORE CLAIMING A NO-OP** | `docs/stub_heatmap.md`, `tools/stub_heatmap.py`, `engine/core/stub_telemetry.py` | **Trigger: any time you suspect — or are about to assert — that an SDK call is (or isn't) a silent no-op, READ `docs/stub_heatmap.md` FIRST. Never assert stub behaviour from reasoning alone.** It ranks unimplemented attrs by live hit count, plus a **Boolean-test call sites (truthiness risk)** table. **Known blind spot:** it only covers the *instance*-attr path (`TGObject.__getattr__` → `_Stub`). The **App-module** path (`App.<name>` → `_NamedStub`) is tracked by a separate, orphaned in-memory `_StubTracker` that **never reaches the heatmap**, and records only on `__call__` — so an undefined **constant** (`App.<CLASS>.<CONST>`) is invisible: `__getattr__`/`__int__`/`__bool__` record nothing and it degrades to truthy / `int()==0`. This has caused ≥4 real bugs (keyboard `WC_*`; `TGUIObject.ALIGN_*` collapsing every `AlignTo` to `ALIGN_UL`; `EngRepairPane.REPAIR_AREA`; `STTopLevelMenu_GetOpenMenu` making the cutscene menu-drop a no-op). Whole SDK **modules** can also be silently stubbed in the twin stub lists (`tools/mission_harness.py` AND `tests/conftest.py` — fix BOTH). |
