...
| BC content paths | `engine/paths.py`, `docs/superpowers/specs/2026-09-05-bc-path-resolution-design.md`, `docs/superpowers/specs/2026-09-06-first-run-folder-picker-design.md` | Where `game/` and `sdk/` live. Five sources (CLI flag, env var, `settings.json`, a picked root, legacy in-project), highest **set** one wins even when invalid. Resolved at USE, never at import — a module-level constant is stale the moment the first-run picker changes a root. When no source resolves, boot hands off to the CEF-rendered "Select Bridge Commander Install" screen (`engine/ui/first_run_panel.py`, driven from `engine/host_loop.py`'s own pump loop before the game loop exists) rather than a bare dialog. Guarded by `tests/unit/test_path_indirection.py`. |
