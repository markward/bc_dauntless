...
public:
    ...
    /// crater needs several model units of depth. 20 model units = 0.2 GU
    /// (~35 m, ~11% of hull radius) is a deep accumulated/ram gouge; a single
    /// torpedo deposits ~5-15. (60 read as comically elastic and exaggerated
    /// seam splits between adjacent NiTriShapes; 1.0 was an invisible dimple.)
    /// Tuned by eye against the live renderer together with the shader's
    /// RUPTURE_MIN/MAX (opaque.frag).
    static constexpr float kMaxDepth = 20.0f;
