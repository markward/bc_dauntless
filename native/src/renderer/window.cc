...
void ensure_glfw() {
    if (g_glfw_users.fetch_add(1) == 0) {
        ...
        // the configured game root, default "game"). Letting GLFW hijack the
        // cwd breaks every subsequent relative asset load — and leaks across
        // pytest tests in one process. Opt out.
