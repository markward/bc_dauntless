...
std::vector<BoundSphere> compute_model_bounds(const assets::Model& model) {
    ...
    for (std::size_t i = 0; i < model.nodes.size(); ++i) {
        ...
        for (int mesh_idx : node.meshes) {
            ...
            const auto& cpu = model.meshes[mesh_idx].cpu_data();
            if (!cpu) continue;
            if (cpu->bound_radius <= 0.0f) continue;   // not a volume
            // Radius takes the node chain's uniform scale, read off a basis
            // vector. BC's node transforms are rotation + uniform scale +
            // translation, so one factor is exact rather than approximate.
            const glm::mat3 basis(node_world[i]);
            out.push_back(BoundSphere{
                .center = glm::vec3(node_world[i] *
                                    glm::vec4(cpu->bound_center, 1.0f)),
                .radius = cpu->bound_radius * glm::length(basis[0]),
            });
        }
    }
    return out;
