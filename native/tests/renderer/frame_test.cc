...
// Ambient is dimmed to 0.64 (~-36%, 0.8×0.8) on the exterior view when filmic
// is on, full (×1.0) when off. The exterior-only scope is enforced at the host
// call site; this just pins the scale the helper returns for each toggle state.
TEST(DauntlessFilmicToggle, AmbientScaleTracksToggle) {
    dauntless_filmic::set_enabled(true);
    EXPECT_FLOAT_EQ(dauntless_filmic::ambient_scale(), 0.64f);
    dauntless_filmic::set_enabled(false);
