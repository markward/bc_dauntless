...
class _FixImplicitRelativeImport(ast.NodeTransformer):
    ...
    def visit_Import(self, node):
        ...
        if "." in name or alias.asname is not None or name == self._self_name:
            return node
        if not any((d / (name + ".py")).exists() for d in self._sibling_dirs):
            return node
...
def _implicit_relative_fixer(file_path, logical_rel):
    ...
    siblings being there. Mirrors tests/conftest._implicit_relative_fixer.
    """
    p = Path(file_path)
    sdk_scripts = _sdk_scripts()
...
def _mods_may_override(fullname):
    ...
    unchanged. Mirrors tests/conftest._mods_may_override."""
...
def _mod_override_spec(fullname, logical, qualified=None):
    ...
    module object. Mirrors tests/conftest._mod_override_spec."""
...
def _stock_dotted_name(logical, leaf_name):
    ...
    tests/conftest._stock_dotted_name."""
    parts = logical.split("/")
    out = []
    cur = _sdk_scripts()
    for part in parts[:-1]:
