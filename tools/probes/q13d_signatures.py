###############################################################################
# q13d_signatures -- VEIN 2 (THE PRIZE): method signatures via docstrings
#
# Question(s): docs/instrumented_experiments/2026-07-13-abi-surface-probe.md
#              For every method on every App class, dump im_func.__doc__. If BC's
#              SWIG build embedded C prototypes, this turns the whole method
#              surface into a TYPED api reference. If docstrings are empty/None,
#              they were stripped -- and that itself is the definitive answer.
# Needs combat state? NO -- static surface, run once at the boot menu.
# Output:      game/BCProbe_q13d.cfg  (+ _<k>.cfg chunks if _CHUNK=1)
#
# Run in the -TestMode REPL with:  execfile('q13d_signatures.py')
###############################################################################
...
_CHUNK_SIZE = 400
_MAX_CHUNKS = 100
_DOC_CAP    = 300           # truncate a docstring to this many repr chars

_cfg      = App.g_kConfigMapping
_CFG_BASE = "BCProbe_q13d"
_SECTION  = _CFG_BASE
...
def _flush_chunked():
    ...
    print "done (q13d, chunked, %d files)" % nchunks
...
def _doc_of(m):
    ...
    s = repr(d)                       # repr escapes newlines/'='/control chars
    if len(s) > _DOC_CAP: s = s[:_DOC_CAP] + "...(trunc)"
    return s

# === PROBE BODY ================================================================
try:
    print "q13d: scanning classes ..."
    _dir_app = []
    ...
    _class_names.sort()
    _ntotal = len(_class_names); print "q13d: %d classes" % _ntotal

    _blocks = []; _total = 0; _idx = 0; _nonempty = 0
    for _cname in _class_names:
        _idx = _idx + 1
        if (_idx % 100) == 0: print "q13d: %d/%d" % (_idx, _ntotal)
        ...
        for _attr in _class_attr_names(_cls):
            ...
            if type(_av) in _SCALARS: continue
            _doc = _doc_of(_av)
            if _doc != "None": _nonempty = _nonempty + 1
            _lines.append("App.%s.%s = %s" % (_cname, _attr, _doc))
        _lines.sort()
    ...
    _record("methods_with_nonempty_doc", _nonempty)   # THE VERDICT: >0 => prize is real
    ...
    else: _flush_single(); print "done (q13d) -- nonempty docs: %d" % _nonempty
except Exception, _err:
    _record("FATAL", "%s: %s" % (_exc_name(_err), str(_err))); _flush_single(); print "done (fatal, q13d)"
