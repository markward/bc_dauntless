###############################################################################
# q05b_face_zero_light.py
#
# Question: Q5 from 2026-06-29-weapon-exchange-console-probe.md, LIGHT
#   intensity variant.  With the facing shield pre-set to 0 AND phasers at
#   LIGHT power (PP_LOW, "max subsystem damage"), where does a weapon hit
#   go -- proportional hull/subsystem, mostly subsystem, or otherwise?
#
#   Companion probe: q05a_face_zero_full.py runs the same experiment at
#   FULL intensity (PP_HIGH, "max hull damage").  Run BOTH and compare.
#
# Design choices:
...
#   - Set PhaserSystem.SetPowerLevel(PP_LOW) explicitly so we KNOW the
...
#   2.  PAUSE  (press P)
#
#   3.  execfile('q05b_face_zero_light.py')          <-- PRE pass.
#       Sets your phasers to LIGHT intensity, snapshots subsystem
...
#
#   5.  execfile('q05b_face_zero_light.py')          <-- POST pass.
#       Probe diffs and reports where damage went (hull, which subsystems).
#
# Output: game/BCProbe_q05b.cfg
###############################################################################
...
_SECTION = "BCProbe_q05b"
_CFG_FILE = "BCProbe_q05b.cfg"
_INTENSITY_LABEL = "LIGHT (PP_LOW)"
_log = []
...
try:
    ...
    if _missing is not None:
        ...
        _has_pre = globals().has_key('_q05b_pre')
        ...
        if not _has_pre:
            ...
            # Set phaser intensity to LIGHT (max subsystem damage).
            _call("ws.SetPowerLevel(PP_LOW)", _ws,
                  "SetPowerLevel", (App.PhaserSystem.PP_LOW,))
            ...
            print "  5.  execfile('q05b_face_zero_light.py')"
            print ""
            ...
            _section("Q5 finding (LIGHT intensity)")
            # Use GetDamage() growth as the canonical signal -- it accumulates
            # regardless of parent-vs-child condition rollup behaviour.
            ...
            if d_gt <= 0:
                ...
                conclusion = ("LIGHT INTENSITY -> HULL-ONLY.  "
                ...
                conclusion = ("LIGHT INTENSITY -> HULL+SUBSYSTEMS SPLIT.  "
                ...
                conclusion = ("LIGHT INTENSITY -> SUBSYSTEM-ONLY?  "
                              "d_hull=%.3f, total sub=%.3f. Inspect data."
                              % (d_hull, total_sub_damage))
            _record("Q5 LIGHT CONCLUSION", conclusion)

            del globals()['_q05b_pre']
            _flush()

            _banner("Q5b RESULT (LIGHT intensity)")
            print ""
            print "  " + conclusion
            print ""
            print "Both modes captured.  Run:"
            print "    uv run python tools/probes/collect.py q05"
            print ""
