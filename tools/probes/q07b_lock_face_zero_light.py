###############################################################################
# q07b_lock_face_zero_light.py
#
# Question: Q7 from 2026-06-29-weapon-exchange-console-probe.md, LIGHT
#   intensity variant.  With all shield faces at 0 AND a deliberate
#   subsystem lock set, where does damage go in LIGHT mode?  Does the lock
#   change WHICH subsystem takes damage (vs the 'sensors' default observed
#   in q05b)?
#
#   q05b (face@0, no lock, LIGHT) measured 0% hull / 100% sensors.
#   q07b (face@0, locked,  LIGHT) measures the same with a lock.
#   The diff between them is the lock's effect under LIGHT intensity.
#
# Companion: q07a runs the same experiment at FULL intensity.
#
...
#   2.  PAUSE  (press P)
#
#   3.  execfile('q07b_lock_face_zero_light.py')             <-- PRE pass.
#       Disables target engines/weapons, drops ALL shield faces to 0,
#       sets phaser intensity to LIGHT, locks a subsystem on the target.
#
#   4.  UNPAUSE (P).  HOLD FIRE for about 3 seconds.  PAUSE (P).
#
#   5.  execfile('q07b_lock_face_zero_light.py')             <-- POST pass.
...
_SECTION = "BCProbe_q07b"
_CFG_FILE = "BCProbe_q07b.cfg"
...
try:
    ...
    if _missing is not None:
        ...
        _has_pre = globals().has_key('_q07b_pre')
        ...
        if not _has_pre:
            ...
            if _lock_sub is None:
                ...
                print "  5.  execfile('q07b_lock_face_zero_light.py')"
                print ""
            ...
            _section("Q7 finding (face@0, locked = %s, LIGHT intensity)"
                     % _pre['lock_label'])
            ...
            if d_gt <= 0:
                ...
                conclusion = ("LIGHT+LOCKED: hull=%.3f (%.0f%%), total sub=%.3f (%.0f%%); %s"
                              % (d_hull, hull_pct, total_sub_damage, sub_pct, where))
            _record("Q7b CONCLUSION", conclusion)

            del globals()['_q07b_pre']
            _flush()

            _banner("Q7b RESULT (LIGHT intensity, locked = " + _pre['lock_label'] + ")")
            print ""
            print "  " + conclusion
            print ""
            print "Both Q7 modes captured.  Run:"
            print "    uv run python tools/probes/collect.py q07"
            print ""
