...
**Known gap (being relayed):** the exact conditions inside `Classify1`(`0x0066C9E0`) /
`Classify2`(`0x0066C860`) — which category pairings stop the existing animation vs. reject the new
one vs. coexist — are marked "collaborators not fully specified" in tier-0. Per Mark (2026-07-21) we
proceed now with a faithful-in-spirit conflict model (§5) and relay a precise question to the RE
project in parallel; if the answer differs we refine. **The question is recorded verbatim in
Appendix A.**
...
The conflict model (`Classify1`/`Classify2`) is implemented per §5 (faithful-in-spirit now).
...
## 5. Conflict-resolution model (faithful-in-spirit)

Until the Appendix-A answer arrives, `Classify1`/`Classify2` are implemented from the well-specified
tier-0 rules:

- **Category interruptability:** interruptable = `{CAT_BREATHE 0, CAT_INTERRUPTABLE 1, CAT_GLANCE 5,
  CAT_GLANCE_BACK 6}`; non-interruptable = `{CAT_NON_INTERRUPTABLE 2}`.
- **`ShouldPlayNow`** (tier-0 §4.8): category 2 always plays; a pending move-target blocks anything
  except category 4; with no glance-target it plays; categories 6 and 3 play; otherwise defer.
- **Conflict rule:** a new record **stops** an existing record when the existing one is interruptable
  and the new one is same-or-higher precedence; a new record is **rejected** when a
  non-interruptable (category 2) animation is current/queued and the new one is interruptable;
  otherwise **coexist** (queue). This reproduces the observable SDK behavior (a move/turn locks out
  idle chatter; a glance yields to a move; breaths never interrupt a walk).

The exact `Classify` codes will be reconciled against Appendix A if it lands before merge; the model
is centralized in one function so the refinement is a single edit.
...
- **Proceed now on the `Classify` gap**, relay Appendix A in parallel, refine if it differs.
...
## Appendix A — relayed RE question (Classify1 / Classify2)
