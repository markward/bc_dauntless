...
4. **CRITICAL trigger is state-transition only, and only `IsDisabled` or `IsDestroyed` count.** A non-hull subsystem flipping `IsDisabled` or `IsDestroyed` to `True` for the first time on this tick. `IsDamaged` transitions are deliberately excluded — `IsDamaged()` typically returns `condition < max_condition`, which fires on every subsystem's first damage tick, producing CRITICAL flashes on every first hit. CRITICAL is reserved for "this subsystem stopped working," not "took a scratch." No damage-amount tiebreaker.
