...
ambient light term to **0.3** (−70%; multiply `lighting.ambient` by `0.3`). Off →
...
1. **Helper (testable seam):** add to the `dauntless_filmic` namespace in
   `frame.cc`:
   ```cpp
   constexpr float kFilmicAmbientScale = 0.3f;   // -70% ambient when filmic on
...
|---|---|---|
| Main exterior | ambient ×0.3 + grain/vignette/CA | ambient ×1.0, no filter |
...
- Unit-test `dauntless_filmic::ambient_scale()`: `0.3` when enabled, `1.0` when
...
- Ambient dim = multiply by `0.3` (−70%; live-tuned), named `kFilmicAmbientScale`.
