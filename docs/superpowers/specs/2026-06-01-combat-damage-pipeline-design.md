...
- **Parent subsystems aggregate their children's state.** `IsDamaged = any(child.IsDamaged())`, `IsDisabled = all(child.IsDisabled())`, `IsDestroyed = all(child.IsDestroyed())`. Leaf top-level subsystems (Sensors, Engines, Shield Generator) take direct hits as today.
