...
u_normal_flip_g    int        — global green-channel flip, default 0
```
...
- **Green channel: OpenGL convention (+Y up).** A map authored for the DirectX
  convention (−Y) will read inverted — lighting appears to come from the
  opposite side. `u_normal_flip_g` is a **global** switch rather than a
  per-file heuristic, so there is one documented answer at authoring time and
  no filename magic to remember.
