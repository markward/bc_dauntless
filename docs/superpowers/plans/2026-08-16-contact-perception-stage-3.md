...
**PRECONDITION for stage 4: `STTargetMenu.set_contacts`' `SetNotVisible` branch goes LIVE in stage 4, and today it has no production-path coverage.** Stage 3 leaves `targetable = perceivable and alive_or_wreck and IsTargetable()`, so `targetable ⇒ perceivable` and `_pump_contacts` can never push `perceivable=False, targetable=True`. That makes `target_menu.py`'s `row.SetNotVisible()` unreachable in-game — every listed row is `IsVisible() == 1`, and what actually protects the SDK's `CycleTarget` (and `TargetNearestEnemy`, which uses a non-match as a loop terminator) is the `targetable` gate on the listing, not the visibility flag. Stage 4 **will** decouple the two: a nebula-concealed or beyond-cloak-range contact should stop being *drawn* without ceasing to *exist*. The moment it does, that branch starts executing having only ever been exercised by synthetic `Contact` records in `tests/unit/test_target_menu_visibility_derived.py`.

**How thin that coverage is — mutation evidence (final review, 2026-08-16).** Collapsing `set_contacts`' `if c.perceivable: row.SetVisible() else: row.SetNotVisible()` down to an unconditional `row.SetVisible()` (i.e. deleting the branch stage 4 depends on) fails **exactly two tests**, both in `tests/unit/test_target_menu_visibility_derived.py`: `test_unperceivable_contact_row_is_not_visible` and `test_visibility_follows_a_later_push`. Both hand-build a `Contact(perceivable=False, targetable=True)` record directly — a pairing `perceived_by` cannot currently emit, per the paragraph above. No other test in the suite notices the branch is gone. So the production code path is verifiably dead today (not just "believed unreachable"), and its entire test coverage is synthetic. Stage 4 must not treat those two tests as proof the branch works end to end — it owes a test driven through `_pump_contacts` with a real observer and a real concealed target, not another hand-built record, before relying on `SetNotVisible` in production.
