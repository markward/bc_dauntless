...
**Hard requirement: `STTargetMenu.set_contacts`' `SetNotVisible` branch goes LIVE in stage 4, and today it has no production-path coverage.** Stage 3 leaves `targetable = perceivable and alive_or_wreck and IsTargetable()`, so `targetable ⇒ perceivable` and `_pump_contacts` can never push `perceivable=False, targetable=True`. That makes `target_menu.py`'s `row.SetNotVisible()` unreachable in-game — every listed row is `IsVisible() == 1`, and what actually protects the SDK's `CycleTarget` (and `TargetNearestEnemy`, which uses a non-match as a loop terminator) is the `targetable` gate on the listing, not the visibility flag. Stage 4 **will** decouple the two: a nebula-concealed or beyond-cloak-range contact should stop being *drawn* without ceasing to *exist*. The moment it does, that branch starts executing having only ever been exercised by synthetic `Contact` records in `tests/unit/test_target_menu_visibility_derived.py`. Stage 4 owes it a test driven through `_pump_contacts` with a real observer and a real concealed target — not another hand-built record.
