...
| 06 | [engineer-panel](06-engineer-panel.md) | Engineering: system rows + pillar gauges + tractor/cloak toggles | F5 Engineer; composes power grid + system rows |
...
- **No outer panel borders**. The left edge is a 4 px coloured stripe; the header carries a top-right 14 px rounded corner. The body uses a translucent dark fill so the 3D scene behind reads through subtly.
