...
def test_build_cache_specs_of_overrides_property(fake_ship_two_subs):
    ...
    sub, is_impulse, is_warp, phase, spec, struct = entries[0]
    assert sub is subA
...
def test_build_cache_default_reads_property(fake_ship_two_subs):
    ...
    for sub, _imp, _warp, _ph, _spec, _struct in default_entries:
    ...
    by_sub = {}
    for sub, _imp, _warp, _ph, spec, _struct in default_entries:
