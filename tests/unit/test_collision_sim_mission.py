...
def test_warbird_is_static_and_parked_the_measured_gap_below(spawned):
    ...
    # Measured in PIECE-sphere terms (see the mission docstring): -3.8 leaves
    # the deepest pair 0.32 GU clear at rest; -3.0 already overlapped.
    assert (p.x, p.y) == (0.0, 0.0)
    assert p.z == pytest.approx(collision_sim.ANVIL_Z_GU) and collision_sim.ANVIL_Z_GU == -3.8
