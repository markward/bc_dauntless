...
def test_aggregate_returns_empty_for_none():
    ...
    assert aggregate_for_renderer(None, bc_assets.GAME_ROOT) == []
...
def test_aggregate_returns_empty_for_set_with_no_backdrops():
    ...
    assert aggregate_for_renderer(pSet, bc_assets.GAME_ROOT) == []
...
def test_aggregate_resolves_texture_path_against_game_dir():
    ...
    result = aggregate_for_renderer(pSet, bc_assets.GAME_ROOT)
    assert len(result) == 1
...
def test_aggregate_unresolvable_warning_fires_once_per_set(capsys):
    ...
    aggregate_for_renderer(pSet, bc_assets.GAME_ROOT)
