...
def test_describe_distinguishes_zero_placed_files(tmp_path):
    """A mod with a content root but zero placed files should be flagged."""
    game, sdk = _roots(tmp_path)
    ...
    mods.classify(idx, game, sdk)
    text = mods.describe(idx)
    # Should have "Empty" in the output and some indication that it placed zero files
    assert "Empty" in text
    # Expect either "0 files" or similar wording to indicate the problem
