...
that zeroed _current_speed reset the acceleration ramp from 1.5 GU/s²
* 0.0167s = 0.025 GU/s back to zero — so ships never reached more than
~2 GU/s, far below their 6.3 GU/s MaxSpeed, and they never got within
the 200 GU MidRange where FireScript dispatches phasers.
...
headless — ships stayed ~295 GU apart for the full 15 s of ticking,
distance barely changing despite a setpoint of 6.3 GU/s.
"""
...
def test_in_system_warp_preserves_current_speed():
    ...
    a ship moving at 4 GU/s before the warp is still at 4 GU/s after."""
...
def test_in_system_warp_does_not_re_trigger_after_first_warp():
    ...
    GU past the warp threshold or the target switches — clobbering
    the AI body's intercept motion. See M2Objects live-path diagnostic
    where ships orbited at ~290 GU, never closing into firing range."""
    ship = _make_ship(0.0, 0.0, 0.0)
    target = _make_ship(0.0, 1000.0, 0.0)
