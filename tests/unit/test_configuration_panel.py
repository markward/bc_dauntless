...
def test_initial_settings_round_trip_to_render_payload():
    ...
    assert body["settings"] == {
        "dust_on": False, "specular_on": True, "hdr_on": True, "rim_on": False,
        "decals_on": False, "fxaa_on": True, "procedural_damage_on": False,
        "fov_deg": 62,
    }
...
def test_focus_first_up_lands_on_last_focusable():
    ...
    payload = p.render_payload()
    body = json.loads(payload[len("setConfigurationPanel("):-2])
    assert body["focused"] == 8  # ctrl:procedural_damage is last in a 9-item list
...
def test_toggle_procedural_damage_fires_applier_and_flips_state():
    setter = Mock()
    p, _ = _make(
        initial_settings=SettingsSnapshot(
            dust_on=True, specular_on=True, hdr_on=True, rim_on=True,
            decals_on=True, fov_deg=70, procedural_damage_on=False,
        ),
        set_procedural_damage=setter,
    )
    p.open()
    assert p._settings.procedural_damage_on is False
    assert p.dispatch_event("toggle:procedural_damage") is True
    setter.assert_called_once_with(True)
    assert p._settings.procedural_damage_on is True


def test_render_payload_includes_procedural_damage_on():
    p, _ = _make(
        initial_settings=SettingsSnapshot(
            dust_on=True, specular_on=True, hdr_on=True, rim_on=True,
            decals_on=True, fov_deg=70, procedural_damage_on=False,
    ...
    p.open()
    payload = json.loads(p.render_payload()[len("setConfigurationPanel("):-len(");")])
    assert payload["settings"]["procedural_damage_on"] is False


def test_procedural_damage_is_a_graphics_focusable():
    p, _ = _make(
        initial_settings=SettingsSnapshot(
            dust_on=True, specular_on=True, hdr_on=True, rim_on=True,
            decals_on=True, fov_deg=70, procedural_damage_on=False,
    ...
    assert ("ctrl", "procedural_damage") in p._focusables()


def test_space_on_procedural_damage_row_toggles():
    setter = Mock()
    p, _ = _make(
        initial_settings=SettingsSnapshot(
            dust_on=True, specular_on=True, hdr_on=True, rim_on=True,
            decals_on=True, fov_deg=70, procedural_damage_on=True,
        ),
        set_procedural_damage=setter,
    )
    p.open()
    r = _FakeReader()
    focusables = p._focusables()
    steps = focusables.index(("ctrl", "procedural_damage")) + 1
    for _ in range(steps):
        r.press(r.keys.KEY_DOWN); p.handle_input(r)
    r.press(r.keys.KEY_SPACE); p.handle_input(r)
