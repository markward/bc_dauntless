...
SDK requires SetAction(pAction) — a TGAction object. The script stores
its object ID and looks it up in Update() via TGObject_GetTGObjectPtr.
A TGNullAction is a side-effect-free, immediately-completing action,
perfect for proving Update returns a US_* status."""
import pytest
...
def test_run_action_update_returns_valid_status():
    ...
    inst.SetAction(App.TGAction_CreateNull())
    result = inst.Update()
