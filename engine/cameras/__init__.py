...
# exterior FOV widens linearly with the player's FORWARD speed, reaching
...
# 3969 kph), so full impulse ≈ the full boost. Signed: going astern narrows
# on the same curve (−4° at full reverse) — |velocity| would widen for
# both, which read as wrong. Additive on the user's FOV setting;
# deliberately NOT fed to fov_distance_scale, which would dolly the camera
# in to cancel the widening.
SPEED_FOV_BOOST_DEG = 4.0
SPEED_FOV_FULL_KPH  = 4000.0


def speed_fov_boost_rad(forward_speed_gups: float) -> float:
    """Extra vertical FOV, in radians, for a player whose velocity component
    along its own forward axis is `forward_speed_gups` (negative astern).
    Clamped to ±SPEED_FOV_BOOST_DEG."""
    from engine.units import GUPS_TO_KPH
    t = (forward_speed_gups * GUPS_TO_KPH) / SPEED_FOV_FULL_KPH
