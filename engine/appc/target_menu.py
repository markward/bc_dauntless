...
#
# A fully cloaked ship is undetectable, so it must drop off the player's target
# menu (and any lock on it must clear) the instant it finishes cloaking, and
# reappear the instant it starts decloaking. The AI candidate gate
# (sensor_detection.can_detect) already hides cloaked contacts from *AI* target
# selection, but the player's STTargetMenu is rebuilt from raw set membership —
# a cloaked ship never leaves the set, so without this it lingers in the list.
#
# Gate on the same boundary can_detect uses: invisible only while fully CLOAKED
# (ET_CLOAK_COMPLETED removes; ET_DECLOAK_BEGINNING — the transition out of
# CLOAKED — restores). Mid-cloak / mid-decloak stay visible.

def _ship_from_cloak_event(event):
    """Resolve the owning ShipClass from a cloak event (source = the cloaking
    subsystem). Raise-safe; returns None when it can't be resolved."""
    try:
        src = event.GetSource() if event is not None else None
        if src is None:
            return None
        if hasattr(src, "_climb_to_ship"):
            ship = src._climb_to_ship()
            if ship is not None:
                return ship
        getter = getattr(src, "GetParentShip", None)
        return getter() if callable(getter) else None
    except Exception:
        return None


def _clear_player_lock_on(ship) -> None:
    """Drop the player's target lock if it points at ``ship`` (a now-cloaked,
    untargetable contact). Raise-safe — no player / no lock is a no-op."""
    try:
        import App as _App
        player = _App.Game_GetCurrentPlayer()
        pship = player.GetShip() if (player is not None and hasattr(player, "GetShip")) else None
        if pship is None:
            return
        if hasattr(pship, "GetTarget") and pship.GetTarget() is ship:
            pship.SetTarget(None)
    except Exception:
        pass


class _CloakTargetMenuGate:
    """Broadcast handler: prunes / restores a ship's target-menu row as it
    cloaks / decloaks."""

    def OnCloakCompleted(self, event):
        ship = _ship_from_cloak_event(event)
        menu = STTargetMenu_GetTargetMenu()
        if ship is None or menu is None:
            return
        row = menu.GetObjectEntry(ship)
        if row is not None:
            menu.DeleteChild(row)
        _clear_player_lock_on(ship)

    def OnDecloakBeginning(self, event):
        ship = _ship_from_cloak_event(event)
        menu = STTargetMenu_GetTargetMenu()
        if ship is None or menu is None:
            return
        if menu.GetObjectEntry(ship) is None and hasattr(ship, "StartGetSubsystemMatch"):
            menu.RebuildShipMenu(ship)
            menu.ResetAffiliationColors()


_cloak_target_menu_gate = None
_cloak_target_menu_wrapper = None


def install_cloak_target_menu_gate() -> None:
    """Idempotently register the cloak → target-menu broadcast handlers so a
    cloaked ship leaves the player target list (and reappears on decloak).
    Safe to call repeatedly; safe when the event manager is unavailable."""
    global _cloak_target_menu_gate, _cloak_target_menu_wrapper
    if _cloak_target_menu_gate is not None:
        return
    try:
        import App as _App
        gate = _CloakTargetMenuGate()
        wrapper = _App.TGPythonInstanceWrapper()
        wrapper.SetPyWrapper(gate)
        _App.g_kEventManager.AddBroadcastPythonMethodHandler(
            _App.ET_CLOAK_COMPLETED, wrapper, "OnCloakCompleted")
        _App.g_kEventManager.AddBroadcastPythonMethodHandler(
            _App.ET_DECLOAK_BEGINNING, wrapper, "OnDecloakBeginning")
        # Keep strong refs so the wrapper/gate aren't GC'd out of the handler list.
        _cloak_target_menu_gate = gate
        _cloak_target_menu_wrapper = wrapper
    except Exception:
        _cloak_target_menu_gate = None
        _cloak_target_menu_wrapper = None


def reset_cloak_target_menu_gate() -> None:
    """Forget the registered gate so a subsequent install re-registers on the
    current event manager. Called from reset_sdk_globals, which wipes the event
    manager's handler tables on every mission swap."""
    global _cloak_target_menu_gate, _cloak_target_menu_wrapper
    _cloak_target_menu_gate = None
    _cloak_target_menu_wrapper = None
